classdef CKernelComponentAnalysis < handle
    ...
    methods
        ...
        function e = analyze(obj, numSources, maxIteration, weightLambda, verbose)
        ...
        function profile = analyze_init(obj, weightLambda)
            ...
            warning('off', 'all');
            gm = fitgmdist( a', obj.mSourceNum, 'Options', statset('MaxIter',500), ...
                            'Replicates', 10 );
            warning('on');
        ...
        function [idxes] = compare_estimated_and_source( obj, idxes, timeRange )
            ...
            for c = 1 : obj.mSourceNum
                legends{end+1} = ['Est. Source ', num2str(c)];
            end
            for k = 1 : num
                subplot(rows, cols, k);
                plot( scanIdx, ori(scanIdx, k), 'LineWidth', 2 ); hold on;
                plot( scanIdx, est(:, k), 'k-.', 'LineWidth', 4 );
                cidx = 1;
                for c = 1 : obj.mSourceNum
                    temp = sourceWeights(k,c) * obj.mSources.profile(c,:);
                    plot( scanIdx, temp, colors(cidx), 'LineWidth', 2 );
                    cidx = cidx + 1;
