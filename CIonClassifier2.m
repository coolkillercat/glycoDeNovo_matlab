classdef CIonClassifier2 < handle  % By Pengyu Hong @ Brandeis University
    properties
        mBoostNum = 100;
        mHoldOut = 0.2;
    ...
    methods
        function ionScoreMap = predict_ions(obj, testVectors, testData)
            idxX15_B =  abs( obj.mMassFeatures - (-27.9949) ) < 0.005 ;
            idxX15_C =  abs( obj.mMassFeatures - (-27.9949 - CMass.H2O) ) < 0.005 ;
        ...
        function ICScores = score_candidates(obj, spectra)
            [vectors, signals] = CIonClassifier2.extract_data( spectra, obj.mMassFeatures, [], 0.005, obj.mUseOriginalPeaks );
            ...
            for s = 1 : length(spectra)
                ...
                for peakIdx = 1 : spectra(s).num
                    
                topologies = CTopology.empty(0,0);
                % for TSS = spectra(s).mPeaks(end).mInferredSuperSet
                for TSS = spectra(s).mPeaks(peakIdx).mInferredSuperSet    
                    topologies = [topologies, TSS.mTopologies];
                end
                ICScores{s} = cell(1, length(topologies));
                for t = 1 : length(topologies)
                    ...
                    for m = 1 : len
                        ...
                        if isempty( spectra(s).mPeaks(peakID).mIonClassifierScores )
                            spectra(s).mPeaks(peakID).mIonClassifierScores = [0, 0];
                        end
                        ICScores{s}{t}(1, m) = peakID;
                        type = 0;
                        for tssIdx = 1 : length( spectra(s).mPeaks(peakID).mInferredSuperSet )
                            peakTSS = spectra(s).mPeaks(peakID).mInferredSuperSet( tssIdx );
                            if isempty( peakTSS.mFormulas ), continue; end
                            switch peakTSS.mTargetPeaks(2, peakTSS.mTargetPeaks(1, :) == peakID)
                                case {CPeakType.B, CPeakType.B_H, CPeakType.B_2H}
                                ...
                                case {CPeakType.C, CPeakType.C_H, CPeakType.C_2H}
                                    if spectra(s).mPeaks(peakID).mComplement < 0
                                        ICScores{s}{t}(2, m) = -spectra(s).mPeaks(peakID).mComplement;
                                        type = -2;
                                    else
        ...
        function [trainVectors, trainData] = train(obj, spectra)
            [trainVectors, trainData] = CIonClassifier2.prepare_training_data( spectra, [], [], obj.mMassAccuracy, 0, 0 );
    ...
    methods(Static)
        function [testVectors, dataSignals] = extract_data( spectra, massFeatures, massBound, massAccuracy, useOriginalPeak, includeREM )
            ...
            for s = 1 : length(spectra)
                ...
                peakIntensities = CIonClassifier2.standardize_intensity( specU.mPeaks, 'log', peakAvailable == 1 );
                ...
                for k = 1 : length(specU.mPeaks)
                    ...
                    for TSS = specU.mPeaks(k).mInferredSuperSet
                        ...
                        ionType = TSS.mTargetPeaks(2, ( TSS.mTargetPeaks(1,:) == k ));
                        if isempty( ionType )
                            assert(false);
                        end
                        
                        switch ionType
                            case {CPeakType.B, CPeakType.C}
                                pMass = peakMasses(k);
                            case {CPeakType.B_H, CPeakType.C_H}
                                pMass = peakMasses(k) + CMass.H;
                            case {CPeakType.B_2H, CPeakType.C_2H}
            ...
            disp( 'CIonClassifier2.extract_data() Done' );
        end
        ...
        function [dataVectors, dataSignals, massFeatures] = prepare_training_data( spectra, exprMethod, massBound, massAccuracy, useOriginalPeak, includeREM )
            ...
            if nargin < 3 || isempty(massBound), massBound = [CMass.H*1.5, 105]; end % [minimal, maximal]
            if nargin < 4 || isempty(massAccuracy), massAccuracy = 0.005; end
            ...
            for s = 1 : length(spectra)
                ...
                for p = 1 : length(specU.mPeaks)-1
                    % if ~isempty(specU.mPeaks(p).mInferredFormulas)
                    if ~isempty(specU.mPeaks(p).mInferredSuperSet)
                        peakAvailable(p) = 1;
                ...
                for it = 1 : 4
                    ...
                    [uniqueMasses, ~, uID] = unique( round( ionprops.(iontype).masses * 1000 ) / 1000 );
                    ...
                    ionprops.(iontype).uID = uID;
                    
                    % collect signals
                    ionprops.(iontype).uniqueMassAvailable = ones(1, numUniqueMasses); % Use uniqueMassAvailable to avoid duplication
                    for k = 1 : numUniqueMasses
                        ionM = uniqueMasses(k);
                        [minDiff, minIdx] = min( abs( peakMasses - ionM ) );
                        if minDiff < massAccuracy
                            peakAvailable( minIdx ) = 0;
                            ionprops.(iontype).uniqueMassAvailable(k) = 0; % Use uniqueMassAvailable to avoid duplication
                ...
                for k = 1 : length(specU.mPeaks)-1 % This is for -H or -2H
                    if ~peakAvailable(k)
                        continue;
                    end
                    for TSS = specU.mPeaks(k).mInferredSuperSet
                        ...
                        if type == CPeakType.T || type == CPeakType.T_H || type == CPeakType.T_2H
                            continue;
                        end
                        switch type
                            case {CPeakType.B, CPeakType.C}
                        ...
                        if minDiff < massAccuracy && ionprops.(iontype).uniqueMassAvailable(minIdx) % Use uniqueMassAvailable to avoid duplication
                            ...
                            dataSignals.(iontype)(end).REM = hasRequestREM;
                            dataSignals.(iontype)(end).spectrum = spectra(s);
                            dataSignals.(iontype)(end).ions = ions.(iontype)(ionprops.(iontype).uID == minIdx);
                        end
                    end
                end
            ...
            disp( 'CIonClassifier2.prepare_training_data() Done' );
        end
        ...
        % [filteredSignals, filteredVectors] = filter_BC_by_rootmono( dataSignals, dataVectors, rootmonos )
            ...
            disp( 'CIonClassifier2.filter_data_by_rootmono() Done' );
        end
        
        function [zscores, intensities, m, s] = standardize_intensity( peaks, transform, peakFlags )
            if nargin < 2, transform = ''; end
            if nargin < 3, peakFlags = []; end
