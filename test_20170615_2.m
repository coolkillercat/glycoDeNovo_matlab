%% 2017/06/15
clear filename;
datapath = ['D:\Projects\Glycomics\data\', '20170615', filesep];
id = 1; gap = []; minus2H = [];
...
filename{id} = 'A2F.DR.Na.EED.PM.txt'; gap(id) = 0; minus2H(id) = 1; id = id + 1; % 26
filename{id} = 'A2F.Reduced.Na.EED.PM.txt'; gap(id) = 0; minus2H(id) = 0; id = id + 1; % 27
filename{id} = 'NA2F.O18.Na.EED.PM.txt'; gap(id) = 0; minus2H(id) = 0; id = id + 1; % 28
filename{id} = 'LNFP 2.DR.Na.EED.PM.txt'; gap(id) = 0; minus2H(id) = 0; id = id + 1; % 29
...
for f = 28 : 29
    ...
    %
    name = ['rec.' filename{f}];
    specU.save_reconstruction( [datapath, 'results/', name + '2'], [], reconstructor.mCheckMinus2H*2, reconstructor.mCheckGap );
    save( strrep( [datapath, 'results/', name + '2'], '.txt', '.mat' ), 'specU' );
    disp( filename{f} );
end
