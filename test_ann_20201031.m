...
mBoostNum = 15;
mHoldOut = 0.2;
monoList = ["Hex", "HexNAc", "Neu5Ac", "Fuc"];
linkageList = linkageClassifier.mono2linkage(monoList);
...
delete(hp);
%%
massFeature = calculate_mass_feature(lc, SpecSet, masses, massBound, massAccuracy);
lc.mMassFeatures = massFeature;
%% output
%save(SpecSet)?
...
for s = 1 : length(SpecSet)
    ...
    for p = 1:length(spec.mPeaks)
        ...
        if strcmp(peak.type, 'B') == 1 && any(strcmp([peak.RE, peak.NRE], linkageList) == 1)% && (strcmp(peak.RE, 'Hex') == 1 && strcmp(peak.NRE, 'HexNAc') == 1 || strcmp(peak.RE, 'Hex') == 1 && strcmp(peak.NRE, 'Hex') == 1 || strcmp(peak.RE, 'HexNAc') == 1 && strcmp(peak.NRE, 'Hex') == 1)
...
%     testY = [ones(size(posTestX,1),1); zeros(size(negTestX,1),1)];
    ...
    hp{2} = weights;
        ...
        classifierMap(strcat([pos, neg])) = currclassifier;
...
lc.mClassifierMap = classifierMap;
fclose(fid3);
disp('Done!')
%%
lc.linkageList = linkageList;